...
class Inventory(object):
    ...
    def __init__(self, manager):
        ...
        self.disk = 'sda'
        self.itsm_agent = self.manager.getITSMAgen()
        self.logger.info('Running inventory')
        o, e, ec = manager.runInShell('%s --local /tmp --no-category=software,user,process,environment,controller,memory,drive,usb,slot,input,port' % self.itsm_agent)
        ...
        info = fcntl.ioctl(sock.fileno(), 0x8927,  struct.pack('256s', self.interface[:15].encode('utf-8')))
        self.macaddress = ':'.join(['%02x' % ord(char) for char in info[18:24]])
        self.ipaddress = socket.inet_ntoa(fcntl.ioctl(sock.fileno(), 0x8915, struct.pack('256s', self.interface[:15].encode('utf-8')))[20:24])
        self.netmask = socket.inet_ntoa(fcntl.ioctl(sock.fileno(), 0x891b, struct.pack('256s', self.interface.encode('utf-8')))[20:24])
